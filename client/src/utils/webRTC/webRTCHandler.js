...
export const getLocalStream = async () => {
  try {
    ...
    // Enable all local audio tracks (if needed)
    stream.getAudioTracks().forEach((track) => {
      console.log("Enabling local audio track:", track);
      track.enabled = true;
    });
...
const createPeerConnection = () => {
  ...
  peerConnection.ontrack = ({ streams: [stream] }) => {
    ...
    // Enable all remote audio tracks (if needed)
    stream.getAudioTracks().forEach((track) => {
      console.log("Enabling remote audio track:", track);
      track.enabled = true;
    });

    store.dispatch(setRemoteStream(stream)); // Save in your state
  };
...
export const handlePreOffer = (data) => {
  if (checkIfCallIsPossible()) {
    ...
    store.dispatch(setCallerImage(data.callerImage));
...
export const handlePreOfferAnswer = (data) => {
  ...
  console.log("Pre-offer answer data: ", data);
  console.log("Current call state: ", store.getState().call.callState);
  console.log("Local stream: ", store.getState().call.localStream);
